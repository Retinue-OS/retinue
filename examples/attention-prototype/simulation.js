...
(function (root) {
  ...
  const SCRIPT = [
    ...
    { at: t(16, 28), who: 'you', action: { type: 'say', id: 'prj-vat', text: 'Can you file the VAT return for me?' }, text: 'The VAT return has sat in Next all day. You open its thread and ask Ara to file it. She has the figures ready but cannot submit: the portal needs your login. By hand in ten minutes, or a Cowork session with the Ara connector, where Claude fills the form in your browser and asks her for the figures.' },
    { at: t(16, 30), who: 'you', action: { type: 'lead', id: 'prj-vat', lead: 28 * DAY }, text: 'Two weeks is too short a lead for a filing like this, so you correct the lead time to four weeks in the details. The profile learns it for every tax filing, and with the deadline now inside the lead time the return climbs to time-sensitive and moves to Now.' },
  ...
  class Simulation {
    ...
    apply(a) {
      ...
      switch (a.type) {
        ...
        case 'reply': return item && item.state === 'open' ? e.sendChat(a.id, a.text) : false;
        case 'chip': case 'say': return item && item.state === 'open' ? !!e.say(a.id, a.type === 'chip' ? a.label : a.text) : false;
