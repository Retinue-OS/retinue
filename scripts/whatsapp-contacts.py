#!/usr/bin/env python3
"""Query the whatsapp-gateway's read API to resolve a WhatsApp contact.

The WhatsApp sibling of signal-contacts.py — the retinue-side client for the
gateway's token-gated read endpoints. It exists so an agent can resolve a name
(e.g. "Jane Doe") to a WhatsApp number before sending — the contact-lookup step
the messaging-contact-lookup skill requires. The gateway is the sole WhatsApp
contact path and works in scheduled/headless sessions.
...
--all or --contacts skips this and dumps a whole roster.

Usage:
    # Resolve a name — recent chats first, directory as fallback (the default)
    python3 scripts/whatsapp-contacts.py --query doe

    # Force the full contact directory (skip the recent-chats layer)
    python3 scripts/whatsapp-contacts.py --query doe --contacts

    # Dump a whole roster
    python3 scripts/whatsapp-contacts.py --all             # recent chats
    python3 scripts/whatsapp-contacts.py --all --contacts  # contact directory

The base URL defaults to the system account's gateway; pass --url to target a
different account's gateway.
"""
import argparse
import json
import os
...
def _fetch_or_exit(base: str, path: str, timeout: float) -> dict:
    ...
    try:
        ...
        print(f"whatsapp-contacts: gateway returned {exc.code} for {path}: {detail}", file=sys.stderr)
        raise SystemExit(1)
    except (urllib.error.URLError, OSError) as exc:
        print(f"whatsapp-contacts: could not reach gateway at {base}: {exc}", file=sys.stderr)
        raise SystemExit(1)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Resolve a WhatsApp contact via the whatsapp-gateway "
