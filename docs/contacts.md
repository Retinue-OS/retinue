...
- `POST /contacts` with `{chamber, name, handles: [{channel, handle}], sphere?,
  tags?}` creates a person and answers 201.
- `POST /contacts/<id>` with `{name?, sphere?, tags?, add?: [handle], remove?:
  [handle]}` changes one.
...
- **Attention priors are keyed on the display name**, not the person. A rename
  moves them (`_attention_rename_sender`), but the right key would be the
  person IRI.
- **The e-mail triage whitelist and the messenger VIP flag** are still their
  own per-address and per-number lists (`triage_policy`). They could name
  persons.
...
- **There is no address-book page in the dashboard**, only the card and the
  HTTP API.
