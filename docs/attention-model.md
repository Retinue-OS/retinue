...
3. **Modes are moods, with a threshold and an allow-list.** A mode says how interruptible the user is, not what hour it is or what they are working on: *Chores* — interruptions welcome, anything urgent rings; *Focused* — head down, and only what is about the *scope* at hand may ring, where the scope is nothing (flow), one sphere (“all clients”) or one project (“this one”); *Social* — people, not subjects; *Rest & relax* — only critical, the digest waiting for the morning. The shipped names are these four; a deployment renames and adds.
