...
| Route | Does |
|---|---|
| `GET /attention` | the union of open threads, chats and projects as sections Now · Next · Held · Waiting, each row with its three fields explained, plus the mode in force, the next breakpoint and the counts |
| `POST /attention/mode` `{mode}` / `{mode: null}` | set the mode by hand or release it to the schedule; a change is a breakpoint |
| `POST /attention/items/<id>/later` `{when: next\|tomorrow}` | snooze |
| `POST /attention/items/<id>/pull` | pull out of Held ahead of the digest |
| `POST /attention/items/<id>/done` | mark handled (a sent reply does this on chats) |
| `POST /attention/items/<id>/correct` `{importance? due? lead?}` | the three-field correction; writes the prior or the lead time into the profile and re-evaluates |
| `POST /attention/permits` `{sender, mode, on}` | let a sender interrupt in a mode |
| `POST /attention/admit` `{sphere, mode, on}` | change a Focus rule |
| `GET /attention/profile`, `POST /attention/profile` | read and edit the profile |

Item ids are the existing ones (`thread:<id>`, `chat:<id>`, the project URI),
so the chat page, the thread and the project page stay the drill-downs.
...
- A base scheduler job, `attention-sweep`, every 30 minutes and credit-free:
  `attention.sweep` over the open items, `attention.breakpoint` at digest times
  and scheduled mode changes, one `Topic`-collapsed digest push with
  `Urgency: normal`, `Urgency: high` on breakthroughs (`push_notify.py` passes
  both headers to pywebpush). Quiet hours are the Off mode; a manual change is
  a breakpoint immediately.
- The repeat policy per sender class in the gateways' notify rail
  (`attention.repeat_policy`).
- The per-device notification mode retires in favour of the focus mode.
...
## Decisions to take before slice 1 lands

1. The sphere set and the default modes and schedule (the brief's list), now
   as the shipped `focus.json`.
2. Whether the news card stays on the home below the attention list, or the
   home is the list alone with News in the dock.
3. Whether slice 1 already retires the per-device notification modes, or they
   stay one more slice as the second filter.
